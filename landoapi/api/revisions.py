...
def _format_revision(
    ...
    commit_message = _create_commit_message(
        revision['title'], bug_id, reviewers
    )
...
def _build_reviewers(phab, revision_id):
    ...
    return sorted(reviewers, key=lambda k: k['phid'])
...
def _create_commit_message(title, bug_id, reviewers):
    """Helper method to create the initial commit message for a Revision.

    Attributes:
        title: The revision title.
        bug_id: Bug ID in Bugzilla.
        reviewers: List of hashes representing reviewers.

    Returns:
        Formatted commit message
    """
    usernames = [r['username'] for r in reviewers] if reviewers else None

    return format_commit_message(title, bug_id, usernames)
