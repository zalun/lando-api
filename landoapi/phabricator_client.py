...
class PhabricatorClient:
    ...
    def get_reviewers(self, revision_id):
        ...
        Requests `revision.search` to get the reviewers data. Then, using the
        received reviewerPHID keys a new request is made to `user.search`
        to get the user info about the reviewers. A new dict indexed by phid
        is created with keys and values from both requests.

        Attributes:
            revision_id: integer, ID of the revision in Phabricator

        Returns:
            A hash of reviewers updated with the user info indexed by phid.
        """
        ...
        if (not result['data'] or
            not result['data'][0]['attachments']['reviewers']['reviewers']):
            return {}

        reviewers_data = result['data'][0]['attachments']['reviewers'
                                                         ]['reviewers']

        # Get user info of all reviewers
        ...
        if len(reviewers_data) != len(reviewers_info):
            logger.warning(
                {
                    'reviewer_phids': reviewers_phids,
                    'user_phids': [r['phid'] for r in reviewers_info],
