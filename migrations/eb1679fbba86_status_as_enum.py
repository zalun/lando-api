...
def upgrade():
    ...
    conn.execute(
        text("UPDATE landings SET status='job_submitted' WHERE "
             "status='started'")
    )
    enum_type = sa.Enum(LandingStatus)
    enum_type.create(op.get_bind(), checkfirst=False)
    op.alter_column(
        'landings', 'status', type_=enum_type, existing_type=sa.String(30),
...
def downgrade():
    ...
    enum_type = sa.Enum(LandingStatus)
    op.alter_column(
        'landings', 'status', type_=sa.String(30),
    ...
    conn.execute(
        text("UPDATE landings SET status = 'started' WHERE "
